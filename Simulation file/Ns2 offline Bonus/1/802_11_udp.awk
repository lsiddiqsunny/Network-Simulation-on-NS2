...
#			printf("%15.0f\n", $6); #nBytes);
			...
			packet_received[node]+=(nBytes-header);
...
#		avg_energy_per_packet = total_energy_consumption / nReceivedPackets ;
	...
	total_Throughput=0
	for(i=0; i<max_node;i++) {
		if(packet_received[i]!=0){
		printf("%d %15.5f\n", i, (packet_received[i]*8)/rTime);
		total_Throughput+=((packet_received[i]*8)/rTime);
	}
#		total_energy_consumption += energy_consumption[i];
	}
	printf("Total Throughput by all nodes : %15.5f\n",total_Throughput);
	printf("Total Throughput : %15.5f\n",rThroughput);
